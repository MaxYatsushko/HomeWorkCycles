public class Main {
    public static void main(String[] args) {
        task1();
        task2();
        task3();
        task4();
        task5();
        task6();
        task7();
        task8();
        task9();
        task10();
    }

    public static void task1(){
        System.out.println("Демонстрация задачи 1");

        for (int i = 1; i <= 10; i++){
            System.out.println(i);
        }
    }

    public static void task3(){
        System.out.println("Демонстрация задачи 3");

        for (int i = 0; i <= 17; i++){
            if(i % 2 == 0) {
                System.out.println(i);
            }
        }
    }

    public static void task2(){
        System.out.println("Демонстрация задачи 2");

        for (int i = 10; i > 0; i--){
            System.out.println(i);
        }
    }

    public static void task4(){
        System.out.println("Демонстрация задачи 5");

        for (int i = 10; i >= -10; i--){
            System.out.println(i);
        }
    }

    public static void task5(){
        System.out.println("Демонстрация задачи 5");

        for (int i = 1904; i <= 2096; i += 4){
            System.out.println("Год " + i + " является високосным");
        }
    }

    public static void task6(){
        System.out.println("Демонстрация задачи 6");

        for (int i = 7; i <= 98; i += 7){
            System.out.println(i);
        }
    }

    public static void task7(){
        System.out.println("Демонстрация задачи 7");

        for (int i = 1; i <= 512; i *= 2){
            System.out.println(i);
        }
    }

    public static void task8(){
        System.out.println("Демонстрация задачи 8");

        int monthAccumulation = 29_000;
        int totalAccimulation = 0;

        for (int i = 1; i <= 12; i++){
            totalAccimulation += monthAccumulation;
            System.out.println("Месяц " + i + ", сумма накоплений равна " + totalAccimulation + " рублей");
        }
    }

    public static void task9(){
        System.out.println("Демонстрация задачи 9");

        int monthAccumulation = 29_000;
        int totalAccimulation = 0;

        for (int i = 1; i <= 12; i++){
            totalAccimulation += totalAccimulation / 100;
            totalAccimulation += monthAccumulation;
            System.out.println("Месяц " + i + ", сумма накоплений равна " + totalAccimulation + " рублей");
        }
    }

    public static void task10(){
        System.out.println("Демонстрация задачи 10");

        for (int i = 1; i <= 10; i++){
            System.out.println("2*" + i + "=" + i*2);
        }
    }
}